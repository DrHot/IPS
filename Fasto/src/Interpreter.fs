...
let rec evalExp (e : UntypedExp, vtab : VarTable, ftab : FunTable) : Value =
  ...
  | StringLit(s, pos) ->
        match s with
          | "false" -> BoolVal false
          | "true" -> BoolVal true
          | _ -> let exps = List.map (fun c -> CharVal c) (Seq.toList s)
                 ArrayVal (exps, Char)
  | Var(id, pos) ->
  ...
  *)
  ...
  | Not(e1, pos) ->
        ...
        match (res1) with
          ...
          | _ -> invalidOperands "Failmessage, bool-not: " [(Int, Int)] res1 res1 pos
  | Negate(e1, pos) ->
           let res1 = evalExp(e1, vtab, ftab) 
           match (res1) with
             | (IntVal n1) -> IntVal(- n1)
             | _ -> invalidOperands "Negate fail: " [(Int, Int)] res1 res1 pos
  | Equal(e1, e2, pos) ->
  ...
  *)
  | Scan (farg, ne, arrexp, tp, pos) ->
        ...
        match arr with
          | ArrayVal (lst,tp1) ->
               ArrayVal((List.scan(fun acc x -> evalFunArg (farg, vtab, ftab, pos, [acc;x])) nel lst), Array farg_ret_type)
